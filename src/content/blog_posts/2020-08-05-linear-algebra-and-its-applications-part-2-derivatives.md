...
We can use $f(x)$ and $g(x)$ to denote such linear combinations with shorthand $\sum\_{i=0}^{n} a\_ix^i$ for $a\_i \in \mathbb{R}$. Note that $x$ _isn't actually a variable:_ we haven't defined a way to evaluate $x$ and it doesn't change based off the input. (If you're wondering how we evaluate polynomials in the traditional sense, we use something called the _evaluation homomorphism_).
